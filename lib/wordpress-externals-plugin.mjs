...
/**
 * Packages that WordPress does not expose as standalone window.wp.* globals. 
 * They must be bundled by esbuild rather than externalized, otherwise a 
 * non-existent script handle would be added to the generated .asset.php 
 * dependencies.
 */
const wordpressBundledPackages = new Set( [
	'@wordpress/admin-ui',
	'@wordpress/dataviews',
	'@wordpress/fields',
 ...
 */
export function wordpressExternalsPlugin( assetBaseName = 'index', extraDependencies = [] ) {
	return {
		...
		setup( build ) {
			...
			build.onResolve( { filter: /^@wordpress\// }, ( args ) => {
				if ( wordpressBundledPackages.has( args.path ) ) {
					return null;
				}
 ...
 * @return {import('esbuild').Plugin}
 */
export function wordpressModuleExternalsPlugin( assetBaseName = 'view' ) {
	return {
